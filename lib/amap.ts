// 高德地图API服务
const AMAP_JS_KEY = process.env.NEXT_PUBLIC_AMAP_KEY || ''
const AMAP_WEB_KEY = process.env.AMAP_WEB_SERVICE_KEY || process.env.NEXT_PUBLIC_AMAP_KEY || ''
...
  if (!AMAP_WEB_KEY) { // <-- 已修正
    console.warn('AMAP_WEB_SERVICE_KEY or NEXT_PUBLIC_AMAP_KEY 未配置，无法使用高德地图地址查询')
    return '未知位置'
  }
...
  try {
    // 使用转换后的坐标构建URL
    const url = `https://restapi.amap.com/v3/geocode/regeo?key=${AMAP_WEB_KEY}&location=${gcjLng},${gcjLat}&radius=1000&extensions=all&batch=false&roadlevel=1`
...
    
    // 构建批量请求URL
    const url = `https://restapi.amap.com/v3/geocode/regeo?key=${AMAP_WEB_KEY}&location=${locations}&radius=1000&extensions=all&batch=true&roadlevel=1`;
